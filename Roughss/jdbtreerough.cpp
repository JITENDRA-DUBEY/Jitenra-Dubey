...
class Tree
{
    ...
    int heightofTree(Node*p);
     ...
     Node*createTreeFromPreorder(int preorder[],int inorder[],int start,int end);
...
Node* Tree::createTreeFromPreorder(int preorder[],int inorder[],int start,int end){
    static int i=0;
    ...
    t->lchild=createTreeFromPreorder(preorder,inorder,start,pos-1);
    t->rchild=createTreeFromPreorder(preorder,inorder,pos+1,end);
    return t;
}
...
int main()
{
  ...
  int in[]={1,2,4,3,5};
  int pos[]={4,2,1,5,3};
  t.createTreeFromPreorder(in,pos,0,4);        
  t.preorder(t.root);  
